...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('ssd')
    .setDescription('Announce a Server Shut Down (SSD)')
  ...
  async execute(interaction) {
    ...
    const message = interaction.options.getString('message') || 'The session has ended. Thanks for playing!';
    ...
    try {
      ...
      console.error('ER:LC API error during /ssd:', error);
      apiError = error;
    }
    ...
    const container = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        ...
        new TextDisplayBuilder().setContent('# 🔴 Server Shut Down'),
    ...
    if (status) {
      container.addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`**Final player count:** ${status.CurrentPlayers}/${status.MaxPlayers}`)
      );
    } else {
      container.addTextDisplayComponents(
