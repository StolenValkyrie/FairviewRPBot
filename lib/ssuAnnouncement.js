  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        SESSION_PING_ROLE_ID ? `<@&${SESSION_PING_ROLE_ID}>` : '# <:logo:1533740178030723192> Server Start Up'
      )
    );

  if (SESSION_PING_ROLE_ID) {
    container.addTextDisplayComponents(
      new TextDisplayBuilder().setContent('# <:logo:1533740178030723192> Server Start Up')
    );
  }
